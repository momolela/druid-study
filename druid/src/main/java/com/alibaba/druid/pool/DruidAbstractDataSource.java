...
public abstract class DruidAbstractDataSource extends WrapperAdapter implements DruidAbstractDataSourceMBean, DataSource, DataSourceProxy, Serializable {
    ...
    public DruidAbstractDataSource(boolean lockFair){
        lock = new ReentrantLock(lockFair); // 默认 lockFair 为 false

        notEmpty = lock.newCondition(); // 同一把锁上用于获取连接的线程
        empty = lock.newCondition(); // 同一把锁上用于创建连接
    }
    ...
    protected boolean testConnectionInternal(DruidConnectionHolder holder, Connection conn) {
        ...
        try {
            if (validConnectionChecker != null) { // 校验器不为空
                // 校验组件开始校验，传入连接代理对象，校验的sql，校验超时时间
                // oracle 内部用原生的 JDBC 进行 sql 执行进行校验
            ...
            // 没有校验器且连接关闭的话返回 false
            if (conn.isClosed()) {
                return false;
            }

            // 没有校验器且校验语句设置为空的话返回 false
            if (null == validationQuery) {
                return true;
            }

            // 没有各种数据库校验器组件的情况下默认也是采用 JDBC 然后配置查询超时时间去做连接校验
            Statement stmt = null;
            ResultSet rset = null;
            try {
                stmt = conn.createStatement();
    ...
    public PhysicalConnectionInfo createPhysicalConnection() throws SQLException {
        ...
        // 不是明文密码。可以通过回调链接的方式（比如接口、文件）获取密码
