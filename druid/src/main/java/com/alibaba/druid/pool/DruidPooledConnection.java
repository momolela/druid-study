...
public class DruidPooledConnection extends PoolableWrapper implements javax.sql.PooledConnection, Connection {
    ...
    public DruidPooledConnection(DruidConnectionHolder holder){
        ...
        // 使用该连接的线程，后续在连接关闭的时候需要比对判断，如果是相同线程直接关闭，如果不是相同线程异步关闭连接
    ...
    /**
     * 连接关闭
     * 拿到连接执行完 sql 会自动调用到这里
     * 或者自己手动调用 DruidPoolConnection.close()
     * 结果有两种：1、重新放回池子；2、直接关闭物理连接
     * @throws SQLException
     */
    @Override
    public void close() throws SQLException {
        ...
        // 判断执行 close 的当前线程是否就是使用该连接的线程
        boolean isSameThread = this.getOwnerThread() == Thread.currentThread();

        // 不是相同线程，设置 asyncCloseConnectionEnable 为 true
        if (!isSameThread) {
            dataSource.setAsyncCloseConnectionEnable(true);
        }

        // 不是相同线程，采用同步关闭连接，因为不是相同线程的情况，可能一个连接会被多个线程修改从而引发错误，所以需要加锁进行同步的关闭
        // 很疑惑为什么这里不直接用 isSameThread 进行判断，说明在其他地方也会修改 asyncCloseConnectionEnable 的值
        // 如果开启了 removeAbandoned，也需要同步关闭，因为真正 removeAbandoned 是在其他线程（shrink 的定时任务）里面执行的，所以为了不出错，也要同步
        ...
        try {
            ...
            // 如果有filter，先release filter，然后再回收连接
            // filterChain.dataSource_recycle >> filter.dataSource_releaseConnection >> StatFilterContext.getInstance().pool_connection_close(nanos)
